$(document).ready(function () {          
      //调用函数，初始化表格  
      //initTable();  
      menuChecked("#branchAccountMonthBalance");
      initDataTables() ;
      initDatepicker();
...
$("#addAccountBalanceBtn").click(function () {
	window.location.href = "branchAccount/accountBalance/edit?id=";
})
...
function initDataTables() {
	this.dt = $("#accountBalanceListTab").DataTable({
						...
						ajax : function(data, callback, settings) {
							...
							// ajax请求数据
							$.ajax({type : "GET",
									url : "branchAccount/accountBalanceList/data",
									cache : false, // 禁用缓存
									data : param, // 传入组装的参数
									dataType : "json",
						...
						"fnDrawCallback" : function(oSettings) {
							for (var i = 0, iLen = oSettings.aiDisplay.length; i < iLen; i++) {
								...
								$('.accountBalance-delete').on("click",function(){
									var id = $(this).data("id");
									swal({
										title : "确定删除吗？",
										...
										$.ajax({
											url:"branchAccount/accountBalance/delete/"+id,
											type:'get',
											async: false,
											success:function(res){
												if(res==null){
													...
													if(res.resultCode=='0'){
														...
														window.location.href = "branchAccount/accountBalanceList"; 
													}else{
